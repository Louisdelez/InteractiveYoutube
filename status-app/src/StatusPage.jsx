...
import { CheckCircle2, AlertTriangle, AlertOctagon, ExternalLink } from 'lucide-react';
import { get } from './api';
import { t } from './i18n';
      ...
      <div className="status-inner">
        <header className="status-header">
          <div className="status-brand">
            <img src="/koala-tv.png" alt="" className="status-logo" />
            <span className="status-title">{t('status.standalone.title')}</span>
          </div>
        ...
        <footer className="status-footer">
          ...
          <a
            href={import.meta.env.VITE_BRAND_HOME_URL || 'https://koalatv.com'}
            target="_blank"
            rel="noreferrer"
          >
            {t('status.standalone.brand_link')} <ExternalLink size={12} />
          </a>
        </footer>
      </div>
    </div>
